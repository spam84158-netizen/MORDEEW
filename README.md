# WhatsApp MD User Bot (VØRAXIS MD)
...
[Deploy Now](https://VORAXIS-MD.site/) to set up on Koyeb.

### 2️⃣ Render

[Deploy Now](https://VORAXIS-MD.site/) to set up on Render.
...
4. **Clone & install:**
   ...
   ```sh
   git clone https://github.com/lyfe00011/VORAXIS-MD botName
   cd botName
   yarn install
   ```
...
- [Bot Environment Variables](https://VORAXIS-MD.site/)
- [Frequently Asked Questions](https://VORAXIS-MD.site/)

## VØRAXIS MD Web Pairing Panel

This build includes a mobile-first `web/public/index.html` interface and a Node HTTP server at `web/server.js`.
The panel accepts a WhatsApp number, requests a real Baileys pairing code from WhatsApp, displays the returned code, and polls the real connection state.

Run the panel with `npm run web` or let `index.js` start it automatically. Configure `PORT` (default `3000`) and `WEB_HOST` (default `0.0.0.0`).
